...
class Drawing:
    ...
    def make_canvas(self, text_fk, sent_num, vstep=30):
        ...
        filtered_tree = (self.tree_frame[
                             (self.tree_frame["TextFK"] == text_fk) & (
                                         self.tree_frame["sentence_number"] == sent_num)].copy()
                         .reset_index(drop=True))
        ...
        for i in range(len(filtered_tree)):
            ...
            builder.append(
                f"ctx.fillText(\"{self.synttolk(filtered_tree["ct"].values[i])}\", {20 * max_ + 160}, {pto * vstep + 12});")
            builder.append("ctx.fillStyle = '#0b910b';")
            builder.append(
                f"ctx.fillText(\"{self.syntvidntolk(filtered_tree["vidn"].values[i])}\", {20 * max_ + 160}, {pto * vstep + 26});")

        builder.append("ctx.fillStyle = '#000';")
        for i in range(len(filtered_morf)):
            builder.append(
                f"ctx.fillText(\"{filtered_morf["word"].values[i].replace("\"", "'")}\", {20 * max_}, {i * vstep + 12});")
